...
def load_network(opt):
    ...
    if not hasattr(opt, "text_fuse"):
        opt.text_fuse = False
    if any(k.startswith("model_1.text_proj") or "cross_attn" in k for k in state_dict.keys()):
        opt.text_fuse = True

    if opt.views == 2:
        model = two_view_net(opt, opt.nclasses, block=opt.block, return_f=getattr(opt, 'triplet_loss', 0.0))
